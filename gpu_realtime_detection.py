#!/usr/bin/env python3
"""
GPU-Optimized Real-time Object Detection with ONNX YOLO Models
Optimized for RTX 3070 GPU with CUDA acceleration for maximum performance.
"""
...
class GPUONNXYOLODetector:
    """GPU-optimized YOLO detector using ONNX Runtime with CUDA acceleration."""
    ...
    def __init__(self, model_path: str, conf_threshold: float = 0.25, iou_threshold: float = 0.45, 
                 use_gpu: bool = True):
        """
        Initialize GPU-optimized ONNX YOLO detector.
    ...
    def _load_onnx_model(self) -> ort.InferenceSession:
        """Load ONNX model with GPU-optimized execution providers."""
        if self.use_gpu:
            # GPU-optimized providers for RTX 3070
            providers = [
...
class GPURealTimeDetectionApp:
    ...
    def __init__(self, model_path: str, camera_id: int = 0, conf_threshold: float = 0.25, 
        ...
        Args:
            ...
            use_gpu: Whether to use GPU acceleration
        """
        self.detector = GPUONNXYOLODetector(model_path, conf_threshold, iou_threshold, use_gpu)
    ...
    def run(self, show_stats: bool = True):
        ...
        while self.running:
            ...
            if show_stats and frame_count % 5 == 0:  # Update stats every 5 frames
                ...
                gpu_status = "GPU" if self.detector.use_gpu else "CPU"
                
                cv2.putText(result_frame, f"FPS: {fps:.1f} ({gpu_status})", (10, 30),
...
def benchmark_gpu_vs_cpu(model_dir: str = "models", num_frames: int = 100):
    ...
    for model_path in onnx_models:
        ...
        try:
            cpu_detector = GPUONNXYOLODetector(str(model_path), use_gpu=False)
            
            # Warm up
            for _ in range(5):
                cpu_detector.detect(test_image)
            
            # Benchmark
            times = []
            for i in range(num_frames):
                _, inference_time = cpu_detector.detect(test_image)
            ...
            logger.info(f"    CPU: {avg_time:.1f}ms avg, {fps:.1f} FPS")
        ...
        if torch.cuda.is_available():
            ...
            try:
                gpu_detector = GPUONNXYOLODetector(str(model_path), use_gpu=True)
                
                # Warm up
                for _ in range(5):
                    gpu_detector.detect(test_image)
                
                # Benchmark
                times = []
                for i in range(num_frames):
                    _, inference_time = gpu_detector.detect(test_image)
                    times.append(inference_time)
                ...
                logger.info(f"    GPU: {avg_time:.1f}ms avg, {fps:.1f} FPS")
                
                # Calculate speedup
                if 'cpu' in model_results:
                    cpu_time = model_results['cpu']['avg_time_ms']
                    gpu_time = model_results['gpu']['avg_time_ms']
    ...
    for model_name, model_results in results.items():
        ...
        if 'cpu' in model_results:
            ...
            logger.info(f"CPU:  {cpu['avg_time_ms']:6.1f}ms | {cpu['fps']:5.1f} FPS")
        
        if 'gpu' in model_results:
            gpu = model_results['gpu']
            logger.info(f"GPU:  {gpu['avg_time_ms']:6.1f}ms | {gpu['fps']:5.1f} FPS")
...
def main():
    parser = argparse.ArgumentParser(description="GPU-Accelerated Real-time Object Detection")
    ...
    if args.benchmark:
        benchmark_gpu_vs_cpu(args.models_dir, args.benchmark_frames)
    else:
