    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if rtsp_url.startswith('http'):
                ...
                try:
                    pipeline = f"rtspsrc location={rtsp_url} latency=0 ! rtph264depay ! h264parse ! avdec_h264 ! videoconvert ! appsink drop=true max-buffers=1"
...
class VideoFileCameraAdapter(CameraAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Successfully opened video file for camera {self.config.id} "
    ...
    def _get_frame_sync(self) -> tuple[Optional[np.ndarray], Optional[FrameMetadata]]:
        """Read/decode a video frame off the event loop."""
        if not self.cap or not self.cap.isOpened():
            return None, None
...
class CameraManager:
    ...
    def _encode_display_frame(self, frame: np.ndarray, camera_id: str) -> bytes:
        """Encode display JPEG and push to Redis SET + PUBLISH in one call.

        This runs in a thread (GIL released during imencode).
        Returns the raw JPEG bytes so the AI path can reuse if needed.
        """
        try:
            orig_h, orig_w = frame.shape[:2]
            disp_frame = frame
            if orig_w > 960:
                disp_frame = cv2.resize(frame, (960, int(orig_h * 960 / orig_w)),
                                        interpolation=cv2.INTER_LINEAR)
            _, disp_buf = cv2.imencode('.jpg', disp_frame, [cv2.IMWRITE_JPEG_QUALITY, 78])
            disp_bytes = disp_buf.tobytes()

            # Atomic set + publish — backend subscriber gets it immediately
    ...
    def _encode_ai_frame(self, frame: np.ndarray, metadata: FrameMetadata) -> bytes:
        """Encode AI-quality frame as base64 JSON for the inference queue."""
        try:
            orig_h, orig_w = frame.shape[:2]
            ai_frame = frame
            if orig_w > 640:
                ai_frame = cv2.resize(frame, (640, int(orig_h * 640 / orig_w)),
                                      interpolation=cv2.INTER_LINEAR)
            _, ai_buf = cv2.imencode('.jpg', ai_frame, [cv2.IMWRITE_JPEG_QUALITY, 70])
            ai_b64 = base64.b64encode(ai_buf.tobytes()).decode('utf-8')
            frame_data = {
                'camera_id': metadata.camera_id,
                'frame_id': metadata.frame_id,
                'timestamp': metadata.timestamp.isoformat(),
                'modality': metadata.modality,
                'resolution': list(ai_frame.shape[:2]),
                'frame_number': metadata.frame_number,
                'metadata': {
                    **metadata.metadata,
                    'detection_enabled': getattr(
                        self.camera_configs.get(metadata.camera_id), 'detection_enabled', True
                    )
                },
                'image_data': ai_b64
            }
            ...
            logger.error(f"AI encode failed ({metadata.camera_id}): {e}")
            return b""
    ...
    def _serialize_frame(self, frame: np.ndarray, metadata: FrameMetadata) -> bytes:
        ...
        return self._encode_ai_frame(frame, metadata)

    def _store_frame_data(self, camera_id: str, frame_data: bytes):
        """Push AI-quality frame to inference queue only."""
        pipe = self.redis_client.pipeline()
    ...
    async def _capture_camera_frames(self, camera_id: str):
        ...
        while self.running and config.enabled:
            try:
                ...
                # This encodes ONE JPEG (78q, ≤960px) and does:
                #   redis.set(latest_frame_jpg:{id})  → snapshot fallback
                #   redis.publish(display_frame:{id}) → MJPEG subscribers get it NOW
                await asyncio.to_thread(self._encode_display_frame, frame, camera_id)

                # ── STEP 2: AI encoding runs in background (never delays display)
                # Encodes a second 640px JPEG as base64 JSON for the inference queue.
    ...
    async def _ai_encode_and_queue(self, frame: np.ndarray, metadata: FrameMetadata):
        ...
        try:
            ai_data = await asyncio.to_thread(self._encode_ai_frame, frame, metadata)
            if ai_data:
