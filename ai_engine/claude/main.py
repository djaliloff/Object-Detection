...
# ─────────────────────────────────────────────────────────────────────────────
# MJPEG stream: how often to yield a frame when no new one is available
MJPEG_IDLE_SLEEP    = float(os.getenv("MJPEG_IDLE_SLEEP", "0.02"))   # seconds (50 fps cap)
# WS detection broadcast: min interval between broadcasts per camera
...
class ConnectionManager:
    def __init__(self):
        ...
        self.camera_subscriptions: dict = {}
...
async def stream_camera_mjpeg(camera_id: str):
    ...
    OPTIMISATION: only yield when a *new* frame arrives (pointer comparison).
    Idle sleep of MJPEG_IDLE_SLEEP (default 20 ms → 50 fps cap) avoids
    burning CPU on a tight loop while still feeling live.
    """
    BOUNDARY = b"--frame\r\n"
    ...
    async def frame_generator():
        ...
        while True:
            try:
                ...
                if frame_bytes:
                    idle_cycles = 0
                    # ── OPTIMISATION: only send if frame actually changed ──
...
@api_v1_router.get("/users/me", response_model=UserSchema)
async def get_current_user_info(current_user: User = Depends(get_current_active_user)):
    return current_user

@api_v1_router.put("/users/{user_id}", response_model=UserSchema)
...
@api_v1_router.delete("/users/{user_id}")
...
async def create_camera(
    ...
    if redis_client:
        try:
            ...
            logger.warning(f"Failed to publish camera add event: {e}")
...
async def delete_camera(
    ...
    if redis_client:
        try:
            ...
            logger.warning(f"Failed to publish camera delete event: {e}")
...
async def create_camera_group(
    ...
    import uuid
    group = CameraGroup(id=str(uuid.uuid4()), **group_data.model_dump())
    db.add(group)
    db.commit()
    db.refresh(group)
    return group
...
async def create_zone(
    ...
    import uuid
    zone = Zone(id=str(uuid.uuid4()), **zone_data.model_dump())
    db.add(zone)
    db.commit()
    db.refresh(zone)
    return zone
...
async def process_redis_messages():
    ...
    def redis_listener_thread():
        """Synchronous thread: blocking Redis listen() with reconnect."""
        while True:
            rc = redis_client
            if not rc:
                time.sleep(3)
        ...
        try:
            ...
            while True:
                msg = await asyncio.to_thread(
                    event_pubsub.get_message, ignore_subscribe_messages=True
                )
