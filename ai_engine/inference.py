...
class ModelRegistry:
    ...
    def get_model_for_modality(self, modality: str, camera_id: Optional[str] = None) -> str:
...
class InferenceEngine:
    ...
    def _serialize_detections(self, tracked_objects: List[TrackedObject], frame_data: FrameData) -> bytes:
        ...
        if detection_list:
            ...
            if now_ts - last_snap.get(cam_id, 0) >= 30.0:
                try:
                    root_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
                    snapshot_dir = os.path.join(root_dir, "backend_api", "uploads", "snapshots")
                    os.makedirs(snapshot_dir, exist_ok=True)
                    snapshot_filename = f"snap_{cam_id}_{int(now_ts)}.jpg"
                    ...
                    last_snap[cam_id] = now_ts
                    self._last_snapshot_ts = last_snap
                except Exception as e:
    ...
    def _update_stats(self, inference_time: float, detection_count: int, tracked_count: int, camera_id: str):
        ...
        self.stats['avg_inference_time'] = (
            (current_avg * (processed - 1) + inference_time) / processed
        )
    ...
    async def _process_single_camera(self, cam_id: str, frame: FrameData):
        ...
        try:
            ...
            if self.publish_annotated_frames:
                try:
